...
async function render(input) {
  ...
  const mx = Math.max((ds.marginX ?? 0.75), 0.5) * 72;  // Min 0.5 inch horizontal
  const my = Math.max((ds.marginY ?? 0.5), 0.4) * 72;   // Min 0.4 inch vertical
...
// Export for direct require() usage
if (typeof module !== "undefined") {
  module.exports = { render };
}

// CLI mode: read from stdin when run directly
