      ...
      experience.items.length > 0 ? (
        <div key="experience">
          ...
          {experience.items.map((item, i) => (
            <div key={i} data-resume-entry="" style={{ marginBottom: i < experience.items.length - 1 ? "14px" : 0 }}>
              <div
                style={{
                  fontFamily: "var(--resume-font)",
                  fontSize: "var(--resume-body-size)",
              ...
              >
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
                  <span style={{ fontWeight: 600, color: "#222" }}>{item.role}</span>
                  <span style={{ color: "#888", whiteSpace: "nowrap", marginLeft: "12px", fontSize: "calc(var(--resume-body-size) - 0.5pt)" }}>
              ...
              {item.bullets.filter(Boolean).length > 0 && (
                ...
                >
                  {item.bullets.filter(Boolean).map((bullet, j) => (
                    <li key={j}>
                      {bulletChar && <span style={{ marginRight: "6px", color: "#999" }}>{bulletChar}</span>}
                      {bullet}
                    </li>
                  ))}
                </ul>
      ...
      education.items.length > 0 ? (
        <div key="education">
          ...
          {education.items.map((item, i) => (
            <div key={i} data-resume-entry="" style={{ marginBottom: i < education.items.length - 1 ? "10px" : 0 }}>
              <div
                style={{
                  display: "flex",
              ...
              >
                <div>
                  <span style={{ fontWeight: 600, color: "#222" }}>{item.degree}</span>
                  {item.field && <span style={{ color: "#555" }}> in {item.field}</span>}
                </div>
                <span style={{ color: "#888", whiteSpace: "nowrap", marginLeft: "12px" }}>
                  {renderDateRange(item.startDate, item.endDate)}
                </span>
              </div>
              {item.institution && (
                <div style={{ color: "#666", marginTop: "2px", fontSize: "var(--resume-body-size)", fontFamily: "var(--resume-font)" }}>
                  {item.institution}
                </div>
              )}
            </div>
        ...
        <div key="awards">
          ...
          {awards.items.map((item, i) => (
            ...
            >
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
                <div>
                  <span style={{ fontWeight: 600, color: "#222" }}>{item.title}</span>
                  {item.issuer && <span style={{ color: "#666" }}> - {item.issuer}</span>}
                </div>
                {item.date && (
                  <span style={{ color: "#888", whiteSpace: "nowrap", marginLeft: "12px" }}>
              ...
              {item.description && (
                <p style={{ margin: "2px 0 0 0", color: "#666" }}>{item.description}</p>
              )}
            </div>
          ))}
        </div>
      ...
      projects.items.length > 0 ? (
        <div key="projects">
          ...
          {projects.items.map((item, i) => (
            <div key={i} data-resume-entry="" style={{ marginBottom: i < projects.items.length - 1 ? "14px" : 0 }}>
              <div
                style={{
                  display: "flex",
                  justifyContent: "space-between",
              ...
              >
                <div>
                  <span style={{ fontWeight: 600, color: "#222" }}>{item.name}</span>
                  {item.url && (
                    <span style={{ color: "#999", marginLeft: "8px", fontSize: "calc(var(--resume-body-size) - 1pt)" }}>
                      {item.url}
                    </span>
                  )}
                </div>
      ...
      volunteering.items.length > 0 ? (
        <div key="volunteering">
          ...
          {volunteering.items.map((item, i) => (
            <div key={i} data-resume-entry="" style={{ marginBottom: i < volunteering.items.length - 1 ? "14px" : 0 }}>
              <div
                style={{
                  display: "flex",
                  justifyContent: "space-between",
              ...
              >
                <div>
                  <span style={{ fontWeight: 600, color: "#222" }}>{item.role}</span>
                  {item.organization && (
                    <span style={{ color: "#666" }}>, {item.organization}</span>
                  )}
                </div>
                <span style={{ color: "#888", whiteSpace: "nowrap", marginLeft: "12px" }}>
                  {renderDateRange(item.startDate, item.endDate)}
        ...
        <div key="publications">
          ...
          {publications.items.map((item, i) => (
            ...
            >
              ...
              {item.url && (
                <div style={{ color: "#999", fontSize: "calc(var(--resume-body-size) - 1pt)" }}>
                  {item.url}
                </div>
              )}
            </div>
