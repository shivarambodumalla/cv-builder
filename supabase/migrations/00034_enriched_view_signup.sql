-- Recreate view to include signup-derived location fields
drop view if exists user_profile_enriched;
...
select
  ...
  -- Signup-derived location (from IP lookup at first sign-in)
  p.signup_city,
  p.signup_region,
  p.signup_country,
  ...
  -- Profile-level overrides (user-editable)
